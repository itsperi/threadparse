...
repo.csv     : one row per repo (requires --by-repo)
targets.csv  : one row per thread target per file

Usage:
   python stats.py <input.json> [--out-dir <dir>] [--by-repo]

Expected JSON schema (per file entry)
──────────────────────────────────────
{
"unsafe": bool,
"violations": ["SHARED LIST", ...],
"total_lines":    int,   # total Python LOC in the file
"threaded_lines": int,   # LOC considered part of multithreaded code
"detail": {
   "shared_vars": {
   "<name>": { "type": "list|dict|set|…", "classification": "GLOBAL|…" }
   },
   "thread_targets": [
   {
      "name": str,
      "class": str | null,
      "parent_target": str | null,
      "shared_reads":   { "<var>": { "protected": [...], "unprotected": [...] } },
      "shared_writes":  { "<var>": { "protected": [...], "unprotected": [...] } },
      "mutating_calls": { "<var>": { "protected": [...], "unprotected": [...] } }
   }
   ]
}
}
